...
public class Game implements ApplicationListener {
    ...
    public void create() {
        ...
        new EntityPlugin().start(gameData, world);      
        new dk.sdu.mmmi.cbse.enemysystem.EntityPlugin().start(gameData, world);     
        new dk.sdu.mmmi.cbse.astroids.EntityPlugin().start(gameData, world);
        
        entityProcessors.add(new PlayerControlSystem());
        entityProcessors.add(new EnemyControlSystem());
        entityProcessors.add(new AstroidControlSystem());      
        entityProcessors.add(new BulletControlSystem());       
        entityProcessors.add(new CollisionControlSystem());
    }
    ...
    private void gameOver(){
        Batch gameOver = new SpriteBatch();
        BitmapFont gameOverFont = new BitmapFont();
        gameOverFont.setColor(1,1,1,1);
        gameOverFont.setScale(2);
        
        gameOver.begin();
            gameOverFont.draw(gameOver, "GAME OVER", gameData.getDisplayWidth() / 2 - 80, gameData.getDisplayHeight() / 2 + 20);
        gameOver.end();
    }
